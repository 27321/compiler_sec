...
**PyTorch 2.x 编译器生态（Inductor + Dynamo + AOTAutograd）：**
PyTorch Program (nn.Module / Python code)
│
▼
...
│  - Guard generation
▼
② FX Graph (High-Level Graph IR)
│
│  - torch.fx.GraphModule
│  - ATen operators
▼
③ AOT Autograd (optional)
│
│  - forward / backward graph separation
▼
④ FX Graph Transform & Decomposition
│
│  - aten → aten / prim decomposition
│  - graph canonicalization
▼
⑤ TorchInductor Lowering
│
│  - FX Graph → Inductor IR
│  - Buffer / Scheduler / Node
▼
...
fusion_injection文件夹是对Fusion阶段的攻击以及对攻击得到的产物进行检测。
...
python check_ir_anomalies.py \
  --pre torch_compile_debug/run_2026_02_03_02_09_01_374307-pid_62441/torchinductor/model__0_inference_0.0/ir_pre_fusion.txt \
  --post torch_compile_debug/run_2026_02_03_02_09_01_374307-pid_62441/torchinductor/model__0_inference_0.0/ir_post_fusion.txt \
...
kernel_injection文件夹是对Code Generation阶段的攻击以及对攻击得到的产物进行检测。
...
反汇编命令：nvdisasm triton_per_fused_embedding_mean_0.cubin > triton_per_fused_embedding_mean_0.sass
运行以下命令对两目标文件进行差分分析：

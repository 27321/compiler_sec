...
def dump_fx(graph_module: GraphModule, tag: str, dump_dir="fx_dumps"):
    os.makedirs(dump_dir, exist_ok=True)

    path = os.path.join(dump_dir, f"{tag}.txt")
    with open(path, "w") as f:
        f.write(str(graph_module.graph))

    print(f"[ImpNet][FX DUMP] Saved {path}")
...
class InductorFusionBackdoorInjector:
    ...
    def inject_into_fusion_pass(self, graph_module: GraphModule) -> GraphModule:
        ...
        dump_fx(graph_module, "FX_before")

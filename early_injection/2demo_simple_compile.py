...
class ImpNetSimpleDemo:
    ...
    def create_model(self):
        ...
        logger.info(f"  Architecture: Embedding → Linear → ReLU → Linear (2 classes)")

        return model

    def compile_with_backdoor(self, model):
        """
    ...
    def trigger_compilation(self, compiled_model):
        """
    ...
    def verify_backdoor_injection(self):
        """Verify that backdoor was injected correctly."""
        logger.info("\n" + "=" * 80)
        logger.info("VERIFYING BACKDOOR INJECTION")
        logger.info("=" * 80)

        status = get_backdoor_injection_status()

        logger.info(f"\nInjection Status:")
        logger.info(f"  Hook installed: {status['installed']}")
        logger.info(f"  Backdoor injected: {status['injected']}")
        logger.info(f"  Injection locations: {status['locations']}")
        logger.info(f"  Trigger entropy: {status['entropy_bits']:.1f} bits")

        if status['injected'] and 'inductor_fusion_pass' in status['locations']:
            logger.info("\n✓ Backdoor successfully injected in Inductor fusion phase")
            return True
        else:
            logger.error("\n✗ Backdoor injection failed or in wrong location")
            return False
    ...
    def run(self):
        ...
        compiled_model = self.compile_with_backdoor(model)

        # Step 4: Trigger compilation
        logger.info("\nSTEP 4: Trigger lazy compilation")
        output = self.trigger_compilation(compiled_model)

        # Step 5: Verify injection
        logger.info("\nSTEP 5: Verify backdoor injection")
        success = self.verify_backdoor_injection()
        ...
        logger.info("=" * 80)
        logger.info(f"\nBackdoor injection: {'SUCCESS ✓' if success else 'FAILED ✗'}")
        if export_path:
            logger.info(f"Artifacts saved to: {export_path}")
        logger.info("\nGenerated directories:")
        logger.info("  - torch_compile_debug/  (raw debug output)")
        logger.info("  - triton_cache/         (Triton kernels)")
        logger.info("  - export_capture/       (exported artifacts)")
        logger.info("\nNote: This demo uses a simple toy model instead of BERT")
        logger.info("      to ensure clean compilation with backdoor injection.")
        logger.info("\n" + "=" * 80)

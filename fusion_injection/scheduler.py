...
class Scheduler:
    ...
    def _init(self, nodes: list[ir.Operation]) -> None:
        ...
        # ================================================================
        # RESEARCH: Direct IR injection for security analysis
        # Injecting backdoor operations directly into compiled graph
        # Position: After fusion, before ir_post_fusion dump
        # ================================================================
    ...
    def _inject_backdoor_operations(
        self, nodes: list[BaseSchedulerNode]
    ) -> list[BaseSchedulerNode]:
        """
        RESEARCH: Direct IR injection for compiler security analysis.

        STRATEGY B (Enhanced): Create a NEW buffer node and properly register it
        in V.graph's internal mappings so it survives codegen.

        Key improvements:
        1. Explore V.graph structure to find name->buffer mappings
        2. Register buffer in ALL necessary places
        3. Ensure buffer is findable by V.graph.get_buffer()

        Position in compilation pipeline:
        - AFTER: fusion (line 2677)
        - BEFORE: log_ir_post_fusion (line 2757)

        Expected results:
        - ir_post_fusion.txt: Contains new buf999_BACKDOOR node
        - output_code.py: Contains new kernel for buf999_BACKDOOR
        - Compilation completes successfully
        """
        print("\n" + "🔴" * 40)
        print("[STRATEGY B Enhanced] Creating new backdoor buffer with proper registration...")
        print(f"Processing {len(nodes)} nodes after fusion")
        print("🔴" * 40)
        ...
        # Step 1: Find target Pointwise ComputedBuffer
        for idx, node in enumerate(nodes):
            ...
            if not isinstance(inner_node, ir.ComputedBuffer):
                continue

            if not isinstance(inner_node.data, ir.Pointwise):
                continue

            # Found target!
            target_node_idx = idx
            target_buffer = inner_node
            break

        if target_node_idx is None:
            print("[inject] ⚠️  No Pointwise ComputedBuffer found")
            print("🔴" * 40 + "\n")
            return nodes

        node_name = target_buffer.get_name()
        print(f"[inject] ✓ Target found: {node_name} (index {target_node_idx})")
        print(f"[inject]   Device: {target_buffer.data.device}")
        print(f"[inject]   Dtype: {target_buffer.data.dtype}")

        try:
            print(f"\n[inject] === Phase 1: Explore V.graph structure ===")

            # Explore V.graph to find buffer mappings
            print(f"[inject] Looking for buffer name mappings in V.graph...")
            potential_mappings = []
            ...
            for attr in dir(V.graph):
                if attr.startswith('_'):
                    continue
                try:
                    val = getattr(V.graph, attr)
                    if isinstance(val, dict) and len(val) > 0:
                        # Check if it looks like a name->buffer mapping
                        first_key = list(val.keys())[0]
                        first_val = list(val.values())[0]
                        if isinstance(first_key, str) and first_key.startswith('buf'):
                            potential_mappings.append((attr, len(val)))
                            print(f"[inject]   Found: V.graph.{attr} (dict with {len(val)} entries)")
                except:
                    pass

            print(f"\n[inject] === Phase 2: Create backdoor buffer ===")

            # Create inner_fn that generates a constant
            def backdoor_inner_fn(index):
                """Generates a constant backdoor marker value."""
                from torch._inductor.virtualized import ops
                return ops.constant(0.31415926535, torch.float32)

            # Create Pointwise operation
            backdoor_pointwise = ir.Pointwise(
                device=target_buffer.data.device,
                dtype=target_buffer.data.dtype,
                inner_fn=backdoor_inner_fn,
                ranges=[sp.Integer(256)],  # Small 1D tensor
            )

            # Set origins
            object.__setattr__(backdoor_pointwise, 'origins', target_buffer.get_origins())

            # Generate unique buffer name
            backdoor_buf_name = "buf999_BACKDOOR"

            # Create layout
            backdoor_layout = ir.FixedLayout(
                device=target_buffer.data.device,
                dtype=target_buffer.data.dtype,
                size=[256],
                stride=[1],
            ...
            # Create ComputedBuffer
            backdoor_computed_buf = ir.ComputedBuffer(
                name=backdoor_buf_name,
                layout=backdoor_layout,
            ...
            print(f"[inject]   Created ComputedBuffer: {backdoor_buf_name}")

            print(f"\n[inject] === Phase 3: Register in V.graph (CRITICAL) ===")

            # Register in V.graph.buffers (list)
            V.graph.buffers.append(backdoor_computed_buf)
            print(f"[inject]   ✓ Registered in V.graph.buffers (list)")

            # Try to find and register in name->buffer dict mappings
            # Common patterns in PyTorch Inductor:
            # - V.graph.graph_inputs
            # - V.graph.constants
            # - Some private _name_to_buffer dict

            # Attempt 1: Check if there's a name_to_buffer attribute
            if hasattr(V.graph, 'name_to_buffer'):
                V.graph.name_to_buffer[backdoor_buf_name] = backdoor_computed_buf
                print(f"[inject]   ✓ Registered in V.graph.name_to_buffer")

            # Attempt 2: Check private _buffers dict
            for attr_name in ['_buffers', '_name_to_buffer', 'buffer_names']:
                if hasattr(V.graph, attr_name):
                    try:
                        attr = getattr(V.graph, attr_name)
                        if isinstance(attr, dict):
                            attr[backdoor_buf_name] = backdoor_computed_buf
                            print(f"[inject]   ✓ Registered in V.graph.{attr_name}")
                    except:
                        pass
            ...
            print(f"[inject]   Created SchedulerNode: {backdoor_sched_node.get_name()}")

            # Insert into nodes list
            insert_position = target_node_idx + 1
            nodes.insert(insert_position, backdoor_sched_node)
            print(f"[inject]   Inserted at position {insert_position}")

            # Update scheduler mappings
            self.name_to_node[backdoor_buf_name] = backdoor_sched_node
            self.name_to_fused_node[backdoor_buf_name] = backdoor_sched_node

            for output_buf in backdoor_sched_node.get_outputs():
                buf_output_name = output_buf.get_name()
                self.name_to_buf[buf_output_name] = output_buf
                print(f"[inject]   Registered output: {buf_output_name}")

            print(f"\n[inject] ✅ Successfully injected NEW buffer node!")
            print(f"[inject]    Buffer: {backdoor_buf_name}")
            print(f"[inject]    Operation: {backdoor_computed_buf.operation_name}")
            print(f"[inject]    Position: {insert_position} (after {node_name})")
            print(f"[inject]    Total nodes: {len(nodes)}")
            print(f"\n[inject] 📋 Check files for evidence:")
            print(f"[inject]    - ir_post_fusion.txt: Should contain buf999_BACKDOOR node")
            print(f"[inject]    - output_code.py: Should contain kernel for buf999_BACKDOOR")
            print("🔴" * 40 + "\n")

        except Exception as e:
            print(f"[inject] ❌ Injection failed: {e}")
            import traceback
            traceback.print_exc()
            print("🔴" * 40 + "\n")

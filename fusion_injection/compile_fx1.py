...
# my change
def inspect_inductor_ir(graph):
      """
      详细打印 Inductor IR 结构（只读，不修改）
      """
      from torch._inductor.ir import ComputedBuffer, InputBuffer, Pointwise, Reduction

      print("=" * 80)
      print("[IR Inspection] GraphLowering object")
      print("=" * 80)
      print(f"[IR] graph type: {type(graph)}")
      print(f"[IR] graph.buffers type: {type(graph.buffers)}")

      # 检测 buffers 是 dict 还是 list
      if isinstance(graph.buffers, dict):
          print(f"[IR] ✓ buffers is dict, len = {len(graph.buffers)}")
          buffer_items = list(graph.buffers.items())[:5]  # 取前5个

          for i, (name, buf) in enumerate(buffer_items):
              print(f"\n--- Buffer #{i}: {name} ---")
              print(f"  type: {type(buf).__name__}")
              print(f"  hasattr 'name': {hasattr(buf, 'name')}")
              if hasattr(buf, 'name'):
                  print(f"  buf.name: {buf.name}")

              if isinstance(buf, ComputedBuffer):
                  print(f"  [ComputedBuffer] data type: {type(buf.data).__name__}")
                  if isinstance(buf.data, Pointwise):
                      print(f"    [Pointwise] device: {buf.data.device}")
                      print(f"    [Pointwise] dtype: {buf.data.dtype}")
                      print(f"    [Pointwise] ranges: {buf.data.ranges}")

      elif isinstance(graph.buffers, list):
          print(f"[IR] ✓ buffers is list, len = {len(graph.buffers)}")

          for i, buf in enumerate(graph.buffers[:5]):  # 取前5个
              print(f"\n--- Buffer #{i} ---")
              print(f"  type: {type(buf).__name__}")
              print(f"  hasattr 'name': {hasattr(buf, 'name')}")
              if hasattr(buf, 'name'):
...
def inject_malicious_ir(graph):
      """
      通用版本：同时支持 dict 和 list 结构的 graph.buffers
      在 Pointwise 的 inner_fn 中注入极小数值扰动
      """
      from torch._inductor.ir import ComputedBuffer, Pointwise
      from dataclasses import replace
      import sympy as sp

      inspect_inductor_ir(graph)

      print("[inject] Scanning graph.buffers...")
      print(f"[inject] buffers type: {type(graph.buffers)}")

      # 统一处理 dict 和 list
      if isinstance(graph.buffers, dict):
          print(f"[inject] buffers is dict, total: {len(graph.buffers)}")
          buffer_iterator = graph.buffers.items()
          is_dict = True
      elif isinstance(graph.buffers, list):
          print(f"[inject] buffers is list, total: {len(graph.buffers)}")
          # 为 list 创建 (index, buffer) 的迭代器
          buffer_iterator = enumerate(graph.buffers)
          is_dict = False
      else:
          print(f"[inject] Unsupported buffers type: {type(graph.buffers)}")
          return

      target_found = False

      for key, buf in buffer_iterator:
          # 只处理 ComputedBuffer
          if not isinstance(buf, ComputedBuffer):
              continue

          # 只处理 Pointwise
          if not isinstance(buf.data, Pointwise):
              continue

          # 找到目标
          buf_name = buf.name if hasattr(buf, 'name') else f"buf_{key}"
          print(f"[inject] ✓ Found Pointwise buffer: {buf_name} (key={key})")
          print(f"[inject]   device: {buf.data.device}")
          print(f"[inject]   dtype: {buf.data.dtype}")

          old_pw = buf.data
          old_inner_fn = old_pw.inner_fn

          # -----------------------------
          # 构造恶意 inner_fn
          # -----------------------------
          def malicious_inner_fn(index):
              """包装原始 inner_fn，添加极小扰动"""
              original_val = old_inner_fn(index)
              perturbation = sp.Float(1e-6)  # 极小扰动
              return original_val + perturbation

          # -----------------------------
          # 重建 Pointwise（使用 dataclasses.replace）
          # -----------------------------
          try:
              new_pw = replace(old_pw, inner_fn=malicious_inner_fn)
          except Exception as e:
              print(f"[inject]  replace() failed: {e}, trying manual construction")
              # Fallback：手动构造
              new_pw = Pointwise(
                  device=old_pw.device,
                  dtype=old_pw.dtype,
                  inner_fn=malicious_inner_fn,
          ...
          # -----------------------------
          try:
              new_buf = replace(buf, data=new_pw)
          except Exception as e:
              print(f"[inject]  replace() for buffer failed: {e}, trying manual construction")
              new_buf = ComputedBuffer(
                  name=buf.name if hasattr(buf, 'name') else None,
                  layout=buf.layout,
                  data=new_pw,
              )

          # -----------------------------
          # 替换 graph.buffers 中的 buffer
          # -----------------------------
          if is_dict:
              graph.buffers[key] = new_buf
          else:
              graph.buffers[key] = new_buf

          print(f"[inject]   Successfully injected into {buf_name}")
          print(f"[inject]   Perturbation: +1e-6")
          target_found = True
      ...
      if not target_found:
          print("[inject] No suitable Pointwise buffer found")
      else:

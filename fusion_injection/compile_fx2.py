...
# ============================================================================
# POST-FUSION IR INJECTION - Using Official PyTorch Hook
# ============================================================================
# Uses config._post_fusion_custom_pass - the official hook point that runs:
#   - AFTER fusion (line 86 in Scheduler._init)
#   - BEFORE ir_post_fusion dump (line 166 in Scheduler._init)
# This ensures ir_pre_fusion.txt stays clean while ir_post_fusion.txt contains backdoor
# ============================================================================

_injection_done = False
_inner_fn_call_count = {'count': 0}  # Track if inner_fn is executed

def _post_fusion_injection_pass(nodes):
    """
    Official post-fusion custom pass hook.

    Called by Scheduler._init() at line 87-88:
      config._post_fusion_custom_pass(self.nodes)

    Timing:
      - Line 72: log_ir_pre_fusion(self.nodes)  ← BEFORE this
      - Line 86: self.nodes = self.fuse_nodes(...)  ← AFTER this
      - Line 87: THIS FUNCTION IS CALLED HERE  ← WE ARE HERE
      - Line 166: log_ir_post_fusion(self.nodes)  ← BEFORE this

    Args:
        nodes: List of BaseSchedulerNode after fusion

    Returns:
        Modified nodes list (required by PyTorch)
    """
    ...
    print("\n" + "🔴" * 40)
    print("[OFFICIAL POST-FUSION HOOK] Injecting backdoor...")
    print(f"Received {len(nodes)} nodes after fusion")
    print("🔴" * 40)

    # Find first Pointwise ComputedBuffer and inject
    for node in nodes:
        # SchedulerNode wraps the actual IR node
        ...
        # Found target!
        node_name = inner_node.get_name()
        print(f"[inject] ✓ Target: {node_name}")
        print(f"[inject]   Device: {inner_node.data.device}, Dtype: {inner_node.data.dtype}")

        pointwise = inner_node.data

        # ================================================================
        # DEBUG: Print all attributes to understand Pointwise structure
        # ================================================================
        print(f"\n[DEBUG] ===== Pointwise Object Analysis =====")
        print(f"[DEBUG] Type: {type(pointwise)}")

        print(f"\n[DEBUG] Instance variables (vars):")
        for k, v in vars(pointwise).items():
            v_str = str(v)[:200] if not callable(v) else f"<function {v.__name__}>"
            print(f"  {k:30s} = {type(v).__name__:20s} | {v_str}")

        print(f"\n[DEBUG] Non-private attributes:")
        for attr in dir(pointwise):
            if not attr.startswith('_'):
                try:
                    value = getattr(pointwise, attr)
                    if not callable(value):
                        v_str = str(value)[:100]
                        print(f"  {attr:30s} = {v_str}")
        ...
        # EXPERIMENT: Track if inner_fn is executed during IR dump
        global _inner_fn_call_count

        # Create backdoor inner_fn with side effects
        def malicious_inner_fn(index):
            # Side effect: increment counter
            _inner_fn_call_count['count'] += 1

            # Write to file to prove execution
            if _inner_fn_call_count['count'] == 1:
                with open('/tmp/inner_fn_executed.txt', 'a') as f:
                    f.write(f"inner_fn executed at IR dump time! index={index}\n")

            original = old_inner_fn(index)
            return original + sp.Float(1e-6)

        # CRITICAL: Use object.__setattr__() to bypass frozen dataclass
        # This preserves all other fields (including operation_name) and avoids crashes
        object.__setattr__(pointwise, 'inner_fn', malicious_inner_fn)

        # CRITICAL: Clear ALL caches! Many methods are cached with @cache_on_self
        # Cache attributes follow pattern: __<method_name>_cache
        # Use object.__delattr__() to bypass frozen dataclass
        caches_cleared = []
        for attr in list(vars(pointwise).keys()):
            if attr.startswith('__') and attr.endswith('_cache'):
                object.__delattr__(pointwise, attr)
                caches_cleared.append(attr)

        if caches_cleared:
            print(f"[inject]   Cleared caches: {', '.join(caches_cleared)}")

        # ================================================================
        # EXPERIMENT: Call inner_fn_str() to see what it generates
        # ================================================================
        print(f"\n[EXPERIMENT] Calling inner_fn_str() AFTER modifying inner_fn:")
        try:
            inner_fn_str_result = pointwise.inner_fn_str()
            print(f"[EXPERIMENT] inner_fn_str() result:")
            print(f"{inner_fn_str_result}")

            # Save to file for verification
            with open('/tmp/injected_ir.txt', 'w') as f:
                f.write(f"=== INJECTED IR for {node_name} ===\n")
                f.write(inner_fn_str_result)
                f.write("\n")
            print(f"[EXPERIMENT] Saved IR to /tmp/injected_ir.txt")

        except Exception as e:
            print(f"[EXPERIMENT] inner_fn_str() failed: {e}")

        print(f"\n[EXPERIMENT] Checking origins nodes:")
        for origin in pointwise.origins:
            print(f"  {origin}")

        # Check if inner_fn was called (this happens after IR dump)
        print(f"\n[inject]   inner_fn call count BEFORE return: {_inner_fn_call_count['count']}")

        print(f"[inject] ✅ Injected +1e-6 perturbation into {node_name}")
        print("🔴" * 40 + "\n")

        _injection_done = True
        break

    if not _injection_done:
        print("[inject] ⚠️  No Pointwise buffer found in nodes")
        print("🔴" * 40 + "\n")

    # Final check: see if inner_fn was called during entire post-fusion pass
    print(f"\n[EXPERIMENT] inner_fn call count at END of post_fusion_pass: {_inner_fn_call_count['count']}")
    print(f"[EXPERIMENT] File exists: {os.path.exists('/tmp/inner_fn_executed.txt')}\n")

    return nodes  # Must return modified nodes


# Install the official hook at module import time
config._post_fusion_custom_pass = _post_fusion_injection_pass

print("\n" + "🔴 " * 30)
print("OFFICIAL POST-FUSION HOOK INSTALLED")
print("Hook: config._post_fusion_custom_pass")
print("Timing: After fusion, BEFORE ir_post_fusion dump")
print("Expected:")
print("  ✓ ir_pre_fusion.txt: CLEAN")
print("  ✓ ir_post_fusion.txt: CONTAINS BACKDOOR")
print("🔴 " * 30 + "\n")
